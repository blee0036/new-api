...
func OpenAIChatRequestToGeminiGenerateContent(c *gin.Context, textRequest dto.GeneralOpenAIRequest, info *relaycommon.RelayInfo) (*dto.GeminiChatRequest, error) {
	...
	for _, message := range textRequest.Messages {
		...
		for _, part := range openaiContent {
			if part.Type == dto.ContentTypeText {
				...
				base64Data, mimeType, err := relaymedia.ResolveBase64Data(c, source, action)
				if err != nil {
		...
		if shouldAttachThoughtSignature {
			missingImageSignatures := 0
			for i := range parts {
				if parts[i].InlineData != nil && len(parts[i].ThoughtSignature) == 0 && sharedgemini.AttachThoughtSignatureBypass(&parts[i]) {
					missingImageSignatures++
				}
			}
			if missingImageSignatures > 0 {
				logger.LogWarn(c, fmt.Sprintf("gemini thought_signature missing on %d image part(s); role=%s model=%s", missingImageSignatures, message.Role, upstreamModelName))
			}
		}
	...
	if err := sharedgemini.ApplyImageConfigResponseFormatCompatibility(&geminiRequest, upstreamModelName); err != nil {
		return nil, err
	}
