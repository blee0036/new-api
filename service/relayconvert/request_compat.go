...
func OpenAIChatRequestToClaudeMessages(c *gin.Context, textRequest dto.GeneralOpenAIRequest, relayInfo ...*relaycommon.RelayInfo) (*dto.ClaudeRequest, error) {
	var info *relaycommon.RelayInfo
	if len(relayInfo) > 0 {
		info = relayInfo[0]
	}
	return oaichat.OpenAIChatRequestToClaudeMessages(c, textRequest, info)
}
...
func ApplyGeminiImageConfigResponseFormatCompatibility(request *dto.GeminiChatRequest, modelName string) error {
	return sharedgemini.ApplyImageConfigResponseFormatCompatibility(request, modelName)
}
